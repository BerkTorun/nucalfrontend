<template>
  <div class="taskList" :class="{ important: state == 1 }">
    <div class="mainbox">
      <div
        class="title"
        :class="{ withoutline: taskList && taskList.length > 0 }"
      >
        {{ stateName }} Tasks
        <div class="right">
          <select class="form-control" v-model="paginationSize">
            <option v-for="size in getDefinedValues.pagingSizes" :key="size.id">
              {{ size }}
            </option>
          </select>
        </div>
      </div>
      <div class="content" v-if="!taskList || !taskList.length > 0">
        There is no task tagged with <strong>{{ stateName }}</strong>
      </div>
    </div>
    ...
    <template v-if="taskList">
      <div
        class="mainbox"
        :class="state == 0 ? 'dark' : task.level"
        v-for="task in taskList.slice(
          (paginationCurrent - 1) * paginationSize,
          paginationCurrent * paginationSize
        )"
        :key="task.id"
      >
        <div class="title">
          <span
            class="clickable"
            :class="state == 0 ? 'text-dark' : task.level"
            @click="editTask(task)"
          >
            {{ task.name }}</span
          >
          <div class="right">
            ...
            <label class="badge badge-dark mr-1" v-if="state == 0">{{
              stateName
            }}</label>
            <!-- <label
              class="badge badge-primary mr-2 createSubTask"
              v-if="state != 0 && getMaxSubtaskLength > task.subTasks.length"
              @click="createSubTask(task.id)"
              ><i class="fas fa-plus"></i> Subtask</label
            > -->
            <span class="badge badge-primary" :class="task.level">{{
              task.level
            }}</span>
          </div>
        </div>
        <div class="content">
          ...
          >
            <div
              class="subTask"
              :class="[subTask.level, { closedSubtask: subTask.state == 0 }]"
            ...
            >
              <div class="title">
                <span
                  class="clickable"
                  :class="subTask.level"
                  @click="editSubtask(task.id, task.name, subTask)"
                  >{{ subTask.name }}</span
                >
                <label
                ...
                >
                <label
                  class="float-right badge badge-dark mr-1"
                  v-if="state == 0"
                  >{{ stateName }}</label
                >
    ...
    <div class="pagination" v-if="totalPage > 1">
      <button
        ...
        v-show="paginationCurrent > 1"
        @click="paginationCurrent = 1"
      >
        <i class="fas fa-angle-double-left"></i> First
      </button>
      <button
        class="btn btn-sm btn-primary"
        :class="{ 'btn-success': page == paginationCurrent }"
      ...
      <button
        class="btn btn-sm btn-primary"
        v-show="totalPage > paginationCurrent"
        @click="paginationCurrent = totalPage"
      >
